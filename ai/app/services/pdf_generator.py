...
def generate_rti_receipt_pdf(data: dict) -> bytes:
    ...
    reg_no = data.get('regNo', 'DOPT/R/2026/569651')

    # A4 Page dimensions: 595.27 x 841.89 points
    # Using 28pt margins to match the reference layout perfectly
    doc = SimpleDocTemplate(
        ...
        author="Government of India - RTI Information Access Portal"
    )

    story = []
    styles = getSampleStyleSheet()

    # Pixel-matched Typography & Styles
    style_normal = ParagraphStyle('NormalText', parent=styles['Normal'], fontSize=8.5, leading=11.5, textColor=colors.HexColor('#334155'))
    style_bold_label = ParagraphStyle('BoldLabel', parent=styles['Normal'], fontSize=8.5, leading=11.5, fontName='Helvetica-Bold', textColor=colors.HexColor('#475569'))
    style_value = ParagraphStyle('ValueText', parent=styles['Normal'], fontSize=8.5, leading=12, textColor=colors.HexColor('#0F172A'))
    
    style_reg_label = ParagraphStyle('RegLabel', parent=styles['Normal'], fontSize=7.5, leading=9.5, fontName='Helvetica-Bold', alignment=TA_CENTER, textColor=colors.HexColor('#64748B'))
    style_reg_val = ParagraphStyle('RegValue', parent=styles['Normal'], fontSize=19, leading=22, fontName='Helvetica-Bold', alignment=TA_CENTER, textColor=colors.HexColor('#059669'))

    style_sub_label = ParagraphStyle('SubLabel', parent=styles['Normal'], fontSize=7, leading=9, fontName='Helvetica-Bold', alignment=TA_CENTER, textColor=colors.HexColor('#64748B'))
    ...
    style_section_title = ParagraphStyle('SecTitle', parent=styles['Normal'], fontSize=10, leading=13, fontName='Helvetica-Bold', textColor=colors.HexColor('#0B1C3F'))

    # Extract & Sanitize Payload Data
    reg_no = data.get('regNo', 'DOPT/R/2026/569651')
    date_str = data.get('dateStr', '26 Aug 2026, 03:51 PM')
    target_date_str = data.get('targetDateStr', '25 Sep 2026')
    applicant_name = data.get('name', 'Shivam Kumar')
    email = data.get('email', 'shivam.kumar@email.com')
    mobile = data.get('mobile', '+91 98765 43210')
    address = data.get('address', '123, Green Park, New Delhi - 110016')
    txn_id = data.get('txnId', 'TXN51234567890')
    ...
    raw_amount = str(data.get('amount', 'Rs. 10.00'))
    clean_amount = raw_amount.replace('₹', 'Rs. ').replace('Rs.Rs.', 'Rs.').strip()
    if not clean_amount.startswith('Rs.'):
      clean_amount = f"Rs. {clean_amount}"
    ...
    possible_logo_paths = [
        ...
        os.path.join(os.getcwd(), 'web/public/logo.png'),
    ...
    title_para = Paragraph("<b>Government of India</b><br/><font size=10 color='#0B1C3F'><b>RTI Information Access Portal</b></font><br/><font size=6.5 color='#64748B'>An Initiative under the Right to Information Act, 2005</font>", style_normal)
    ...
    header_table = Table(header_table_data, colWidths=[38, 392, 108])
    ...
    sub_col2 = Paragraph("AMOUNT PAID<br/><b>" + clean_amount + "</b>", style_sub_label)
    ...
    reg_card_data = [
        ...
        [Spacer(1, 3)],
        [HRFlowable(width="100%", thickness=0.5, color=colors.HexColor('#F1F5F9'), spaceAfter=4, spaceBefore=0)],
        [Table([[sub_col1, sub_col2, sub_col3]], colWidths=[178, 178, 178])]
    ]
    reg_card_table = Table(reg_card_data, colWidths=[538])
    ...
    app_details_table = Table(app_details_data, colWidths=[150, 388])
    ...
    req_details_table = Table(req_details_data, colWidths=[150, 388])
    ...
    step1_p = Paragraph("<font color='#0D8A44'><b>&bull; Request Submitted</b></font><br/><font size=7 color='#64748B'>Your application has been successfully submitted.<br/>" + date_str + "</font>", style_normal)
    step2_p = Paragraph("<font color='#2563EB'><b>&bull; Request Under Process</b></font><br/><font size=7 color='#64748B'>The PIO officer will review your request.<br/>Within 30 days</font>", style_normal)
    step3_p = Paragraph("<font color='#2563EB'><b>&bull; You Will Receive a Response</b></font><br/><font size=7 color='#64748B'>The response will be sent to your email.<br/>On or before " + target_date_str + "</font>", style_normal)
    ...
    qr_img_io.seek(0)
    qr_reportlab_img = Image(qr_img_io, width=44, height=44)
    ...
    qr_text = Paragraph("<b>Scan to verify request</b><br/><font size=7 color='#2563EB'>rti.gov.in/verify</font>", style_normal)

    qr_box_data = [[qr_reportlab_img, qr_text]]
    qr_box_table = Table(qr_box_data, colWidths=[48, 108])
    ...
    footer_table = Table(footer_table_data, colWidths=[356, 182])
